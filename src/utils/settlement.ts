import { GroupEventExpense } from '../types';
...
/**
 * Вычисляет минимальный набор переводов для погашения всех долгов.
 *
 * Логика:
 * - Каждый участник делит стоимость траты поровну.
 * - Оплативший не обязан быть в списке участников (напр. веган платит за мясо):
 *   в этом случае все участники должны ему свою долю.
 * - Если оплативший входит в участники, его доля считается уже оплаченной —
 *   остальные участники должны ему свои доли.
 */
export function computeSettlement(expenses: GroupEventExpense[]): Transfer[] {
  ...
  for (const expense of expenses) {
    ...
    for (const participantId of expense.participantIds) {
      ...
      // Участник должен оплатившему свою долю

...
import { createGroup, joinGroupByInviteCode, subscribeToUserGroups } from '@/shared/api';
...
export function GroupsListPage({ userId, user, onSelectGroup }: Props) {
  ...
  return (
    <Container>
      ...
      <Dialog
        open={joinOpen}
        onClose={() => { setJoinOpen(false); setJoinError(''); }}
        fullWidth
        maxWidth="xs"
      >
        ...
        <DialogActions sx={{ flexDirection: 'column', alignItems: 'stretch', gap: 1, px: 3, pb: 2 }}>
          ...
          <Button onClick={() => { setJoinOpen(false); setJoinError(''); }} fullWidth>
            Отмена
          </Button>
        </DialogActions>
      </Dialog>
